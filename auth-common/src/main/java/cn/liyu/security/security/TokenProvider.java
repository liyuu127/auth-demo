...
public class TokenProvider implements InitializingBean {
    ...
    public void checkRenewal(String token) {
        ...
        long time = redisUtils.getExpire(ONLINE_KEY + token) * 1000;
        ...
        if (differ <= properties.getDetect()) {
            ...
            redisUtils.expire(ONLINE_KEY + token, renew, TimeUnit.MILLISECONDS);
        }
    }

    public String getToken(HttpServletRequest request) {
        final String requestHeader = request.getHeader(HEADER);
        if (requestHeader != null && requestHeader.startsWith(TOKEN_TYPE)) {
